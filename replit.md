...
- **Development vs Production Limitation**: Printful API requires publicly accessible design URLs. The scheduler only attempts Printful product sync when NODE_ENV=production (defaults to skipping sync if NODE_ENV is not set). This is because design URLs point to quote-it.co which returns 404 if the quote doesn't exist in the production database. When the winner submits shipping info without a synced product, the order status becomes `pending_sync` for later manual processing. Products are auto-synced when the app runs in production mode with the quote in the production database.
- **Printful Order Submission**: Both paid orders and complimentary (winner) orders use `getSyncVariantForSize()` to dynamically fetch the correct variant ID from Printful API based on size selection. This ensures orders always use the correct sync variant IDs rather than hardcoded values.
- **Product Reconciliation**: On startup, the scheduler reconciles Printful products with the database by parsing `external_id` patterns (format: `quote-{quoteId}-{type}` where type is gold/white/white-author/white-noauthor). Products are created in the database only when their linked quote exists, ensuring proper data integrity.
