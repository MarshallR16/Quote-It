...
The backend utilizes Node.js with Express and TypeScript, running on an ESM module system. It provides a RESTful API. The database layer uses Drizzle ORM with Neon serverless PostgreSQL, supporting type-safe operations and connection pooling. Data models include Users (with referral tracking), Quotes, Votes, Products, Orders (with Stripe integration), and Weekly Winners. The Hall of Fame dynamically ranks users based on wins and votes. The referral system uses cryptographically secure 8-character codes for discounts. Authentication is managed via Firebase Authentication for direct user sign-in (Google/Apple/Email) with backend token verification. The AuthRedirectHandler component globally handles OAuth redirects by waiting for Firebase auth state and invalidating queries. Profile completion flow: for OAuth users (Google/Apple) who lack first/last names, backend returns `{requiresProfile: true, email, profileImageUrl}` which triggers ProfileCompletionModal. For email/password signups, the frontend directly calls `/api/auth/complete-profile` with the name parsed from the sign-up form, bypassing the modal entirely. Query invalidation after profile completion creates the account. A Twitter-style following system allows users to follow others, creating "friends" for mutual follows. The app is publicly viewable, with sign-in required only for interactive features.
...
- **Weekly Winner Free Shirt**: Automated complimentary order for authors of winning quotes with exclusive gold text design. Winners receive a black shirt with gold lettering. Three Printful products are created automatically for each winner: (1) gold text with author attribution (inactive, winner exclusive), (2) white text with author attribution (active, for sale), and (3) white text without author (active, for sale). Design endpoint supports `?includeAuthor=false` query parameter to generate designs without author name.
...
- **Store Automation**: Automated weekly winner selection and Printful product creation via node-cron. Printful integration uses a self-hosted SVG design endpoint (`/api/designs/:quoteId/:textColor`) that Printful fetches from the URL configured via `DESIGN_BASE_URL` env var (defaults to production domain). Products are created with all size variants (S-2XL). Mockups are automatically generated after product creation and stored in the database. Connection test runs on server startup to verify Printful API token validity. Admin endpoints include: debug design URL accessibility (`GET /api/admin/debug/design/:quoteId`), manual mockup generation (`POST /api/admin/products/:productId/generate-mockup`), and manual winner trigger (`POST /api/admin/weekly-winner/trigger`).
- **Zero Demo Products Policy**: The system never creates demo/placeholder products. All products MUST be synced with Printful or creation fails with retry. This ensures every product in the database can accept real orders. The scheduler includes retry logic (3 attempts with exponential backoff) for database saves after Printful product creation.
- **Printful Order Submission**: Both paid orders and complimentary (winner) orders use `getSyncVariantForSize()` to dynamically fetch the correct variant ID from Printful API based on size selection. If a product is missing its `printfulSyncProductId`, the order flow auto-fetches it from Printful using the `external_id` pattern before submission, ensuring orders always reach Printful.
- **Product Reconciliation**: Runs on startup AND hourly via cron job to automatically fix any sync gaps. Parses `external_id` patterns (format: `quote-{quoteId}-{type}` where type is gold/white/white-author/white-noauthor). Products are linked to database records when their quote exists. This self-healing mechanism ensures the system recovers automatically from any transient failures.
