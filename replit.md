...
- **Development vs Production Limitation**: Printful API requires publicly accessible design URLs. In development mode (NODE_ENV=development), the design URL points to production (quote-it.co) which may return 404 if the quote doesn't exist in the production database. Printful product creation only succeeds when: (1) running in production with quotes in the production database, OR (2) when the quote exists in both dev and prod databases. The auto-heal process gracefully handles this by logging the limitation and skipping Printful sync in development. When the winner submits shipping info without a synced product, the order status becomes `pending_sync` for later manual processing.
